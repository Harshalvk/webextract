import { AppNodeMissingInputs, IAppNode } from "@/types/appNode.types";
import {
...
// this is the main function which plans the flow of the execution
    ...
    for (const currentNode of nodes) {
      ...
      if (invalidInputs.length > 0) {
        ...
        if (incomers.every((incomer) => planned.has(incomer.id))) {
          ...
          // this means that this particular node has an invalid input which means that
...
function getInvalidInputs(node: IAppNode, edges: Edge[], planned: Set<string>) {
  ...
  for (const input of inputs) {
    ...
    if (requiredInputProvidedByVisitedOutput) {
      // the inputs is required and we have a valid value for it provided by a task
      // that is already planned
      continue;
    } else if (!input.required) {
      // if the input is not required but there is an output linked to it then we
      // need to be sure that the output is already planned
...
function getIncomers(node: IAppNode, nodes: IAppNode[], edges: Edge[]) {
  if (!node.id) {
    return [];
  }
